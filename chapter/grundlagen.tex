...
In Abschnitt~\ref{subsec:modellierung-spiele-suchprobleme} werden grundlegende Begriffe der Spieltheorie eingeführt und Spiele formal als Suchprobleme modelliert. Darauf aufbauend behandelt Abschnitt~\ref{subsec:prinzipien-kompetitive-spiele} zentrale Prinzipien der Entscheidungsfindung in kompetitiven Spielen, die für die Auswahl geeigneter Algorithmen maßgeblich sind. Abschließend werden die vorgestellten Konzepte in Abschnitt~\ref{subsec:spieltheoretische-einordnung-othello} auf das Spiel Othello angewandt und dieses aus spieltheoretischer Sicht eingeordnet.
...
\begin{table}[hbt]	
	...
	\begin{tabular}{c c|c c c c}
		...
		\multirow{4}{*}{\large \textcolor{red}{Berta} \,} & \multirow{2}{*}{kooperiert ($s_{21}$)} & \multirow{2}{*}{$o_1:$} & \multicolumn{1}{l|}{\textcolor{blue}{Anton erhält 50\,€,}} & \multirow{2}{*}{$o_3:$} & \multicolumn{1}{l|}{\textcolor{blue}{Anton erhält 100\,€,}} \\
		 ...
		 & & & \multicolumn{1}{l|}{\textcolor{red}{Berta erhält 100\,€.}} & & \multicolumn{1}{l|}{\textcolor{red}{Berta erhält 0\,€.}}\\ \hline
	\end{tabular}
	
\end{table}

Formal lässt sich die \textbf{Spielsituation} wie folgt beschreiben: Eine \textbf{Menge von Spielern}
$ I = \{1=\text{Anton}, 2=\text{Berta}\} $ verfügt jeweils über zwei \textbf{reine Strategien}
$ s_i $, nämlich \emph{Kooperieren} ($ s_{i1} $) oder \emph{Defektieren} ($ s_{i2} $).
Abhängig von der Wahl der Strategien ergibt sich eine \textbf{Strategiekombination} $ s $, die als
geordnetes Paar, etwa $ (s_1=\text{Kooperieren}, s_2=\text{Defektieren}) $, dargestellt wird.
Der \textbf{Strategieraum}
$ S = S_1 \times S_2 = \{s_{11}, s_{12}, s_{21}, s_{22}\} $ umfasst die Menge aller möglichen
Kombinationen der Strategien $ s_i \in S_i $. Insgesamt existieren somit vier ($2 \times 2$)
mögliche Kombinationen reiner Strategien.
Jede Strategiekombination $ s $ bestimmt ein \textbf{Ergebnis} $ o_i $. Die Menge aller möglichen
Ergebnisse wird als \textbf{Ergebnisraum}
$ O = \{o_1, o_2, o_3, o_4\} $ bezeichnet.
Die Darstellung der Spielsituation in der Form $ \Gamma^{\dagger} = \langle I, S, O \rangle  $ wird als \textbf{Spielform}
bezeichnet und lässt sich beispielsweise in Matrixform, wie in
Tabelle~\ref{tab:golden-balls}, darstellen. Wird diese um eine \textbf{Ergebnisfunktion}
...
\textbf{Game Frame} bezeichnet wird. \autocite[Kap.~2.1]{bonannoGameTheory2024}, \autocite[Kap.~1.2.1]{hollerEinfuehrungSpieltheorie2019}
...
\emph{(Anmerkung zur Notation:
	$o \succeq_i o'$ bezeichnet eine schwache Präferenz;
	$o \succ_i o'$ eine strikte Präferenz;
	$o \sim_i o'$ Indifferenz.)}
...
Auf Grundlage der zuvor eingeführten Konzepte kann die Definition des Game Frames präzisiert werden. Daraus ergibt sich gemäß \autocite[Def.~2.1.3]{bonannoGameTheory2024} die nachfolgende Definition~\ref{def:ordinales-spiel} eines ordinalen Spiels in strategischer Form.
\begin{definition}
...
\begin{table}[hbt]
	...
	\begin{tabular}{r|c c c c}
		...
		\multicolumn{1}{c|}{\textcolor{blue}{$U_1$ (Anton):}} & \textcolor{blue}{$3$} & \textcolor{blue}{$2$} & \textcolor{blue}{$4$} & \textcolor{blue}{$2$} \\
		\multicolumn{1}{c|}{\textcolor{red}{$U_2$ (Berta):}} & \textcolor{red}{$3$} & \textcolor{red}{$4$} & \textcolor{red}{$2$} & \textcolor{red}{$2$} \\
	\end{tabular}
\end{table}

Basierend auf der Repräsentation der Präferenzen durch Nutzenfunktionen lässt sich ein ordinales Spiel in reduzierter Form definieren. Diese in der Literatur verbreitete Darstellung verzichtet auf eine explizite Spezifikation der Ergebnismenge und wird in der folgenden Definition~\ref{def:reduziertes-ordinales-spiel} formalisiert (vgl.~\autocite[Def.~2.1.3]{bonannoGameTheory2024}).
...
% Übergang zu sequentiellen Spielen
% Einführung Othello als Beispiel
% Othello definiert als Spiel
% Zustandraummodell: State, Action, Übergangsfunktion
% Baumdarstellung: Expliziter Präresnations aller SPielverläufe
% Rpckwärtz induktion


% Relevanz für Kapitel 3:
% - Alle Suchalgorithmenoperieren auf exakt diesem Modell

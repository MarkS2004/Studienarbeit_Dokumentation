...
Eine übergreifende Annahme für die Entscheidungsprinzipien ist die angenommene Rationalität beider Spieler. Dies bedeutet, dass jeder konsistente Zielvorstellungen hat und die Entscheidungen logisch aus diesen Zielen ableitet. Für die Spieler selbst ergibt sich daraus eine Nutzenmaximierung. Dies ist von Bedeutung, da dies die Voraussetzung für die Vorhersagbarkeit gegnerischer Züge bildet, um diese in der eigenen Entscheidungsfindung einzubeziehen. Es ist davon auszugehen, dass ein Othello-Spieler keinen Zug durchführen würde, der zu einem Verlust vieler eigener Steine führt, wenn ein alternativer Zug existiert \autocite[Kap.~2.6]{osborneIntroductionGameTheory2004}.
...
Des weiteren ist der Verzweigungsfaktor von Othello für die Analyse des Spielbaums relevant. Dieser bezeichnet die durchschnittliche Anzahl zulässiger Züge die einem Spieler im Laufe einer Partie zur Verfügung stehen. Somit steht er für die durchschnittliche Zahl an Nachfolgerknoten pro Knoten im Spielbaum. Eine Untersuchung der WTHOR Datenbank \autocite{BaseWTHORFederation} ergab, dass Othello einen durchschnittlichen Verzweigungsfaktor von $8.25$ und eine Spiellänge von $59.85$ Zügen besitzt. Ein durchschnittliches Spiel von Othello dauert somit fast so lang wie ein maximal langes mit $64~Felder-4~Anfangssteine=60~Zuege$. Es ergibt sich somit eine Spielbaumkomplexität von $8.25^{59.85}\approx 7.08\cdot10^{54}$ \autocite[Kap.~3.3.2]{nijssenMonteCarloTreeSearch2013}. Dies bedeutet, dass das der Spielbaum aufgrund des hohen Verzweigungsfaktors sehr breit wächst. Daraus folgend können nicht realistisch alle möglichen Zustände überprüft werden. Abgeleitet aus dem Verzweigungsfaktor wird zudem die Suchtiefe in der adversarialen Suche interessant. Obwohl das Spiel deterministisch ist, kann sich der exakte Spielverlauf nicht komplett vorhersagen, da man die gegnerischen Spielzüge nur bedingt durch die eigenen beeinflussen kann. Es wäre, selbst wenn es möglich wäre, nicht sinnvoll im Vorhinein alle möglichen Lösungswege zu bestimmen und nach jedem Zug abzuwägen, ob er weiter oder ein anderer verfolgt werden sollte. Durch die Bestimmung einer Suchtiefe kann der Suchaufwand erheblich reduziert werden, indem nur von jedem Zustand aus die $d$ darauffolgenden Zustände überprüft werden und unter diesen abgewogen wird. Bei 30 eigenen Zügen mit einer Suchtiefe von 3 und dem Verzweigungsfaktor müssten somit nur noch $30\cdot8.25^3\approx 16.800$ Zustände überprüft werden.
...
Das System umfasst Sensoren, Aktoren, einen Mikrocontroller sowie eine integrierte Entwicklungsumgebung. Es ist mit Komponenten des LEGO\textsuperscript{\textregistered} Technic\texttrademark{} Baukastensystems kompatibel und ermöglicht dadurch den Aufbau komplexerer mechatronischer Systeme.
Das SPIKE-System ist in zwei Ausführungen erhältlich, Essential und Prime. In der vorliegenden Arbeit wird die Prime-Variante betrachtet, auf die sich die nachfolgende Beschreibung beschränkt.

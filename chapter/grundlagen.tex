...
Dieses Kapitel vermittelt die zentralen theoretischen Konzepte, die für die Entwicklung eines Othello-spielenden Roboters erforderlich sind. In Abschnitt~\ref{sec:othello} werden der Aufbau des Spiels sowie das Regelwerk von Othello beschrieben. Abschnitt~\ref{sec:spieltheorie} führt grundlegende spieltheoretische Konzepte ein, insbesondere die Modellierung von Spielen als Suchbäume, die Prinzipien kompetitiver Entscheidungen sowie die Einordnung von Othello in diesen Kontext. Diese Grundlagen bilden die Basis für die anschließende Analyse und Implementierung spielstrategischer Algorithmen sowie für den systematischen Entwurf eines Konzepts.
...
Spiele mit diesen Eigenschaften werden häufig in extensiver Form beschrieben, typischerweise durch die Darstellung als Spielbaum. Dabei zeigt sich, dass die vollständige Lösung eines solchen Spiels rechnerisch anspruchsvoll ist, da der zugehörige Spielbaum eine große Anzahl an Knoten und Kanten umfasst. In der Praxis ist es daher erforderlich, Entscheidungen zu treffen, ohne die optimale Lösung exakt zu kennen oder den gesamten Spielbaum vollständig zu durchsuchen, insbesondere bei begrenzten zeitlichen und rechnerischen Ressourcen. Problemklassen mit diesen Eigenschaften werden in der Algorithmik, insbesondere im Bereich der Künstlichen Intelligenz, als adversariale Suche bezeichnet \autocite[Kap.~5.1]{russellArtificialIntelligenceModern2016}. Die grundlegenden Konzepte adversarialer Suchverfahren als auch die konkreten Algorithmen und deren Anwendung auf das Spiel Othello werden in Kapitel~\ref{cha:stand-der-technik} erläutert.

\subsection{Spieltheoretische Einordnung von Othello}
\label{subsec:spieltheoretische-einordnung-othello}

Eine übergreifende Annahme für die Entscheidungsprinzipien ist die Rationalität beider Spieler. Dies bedeutet, dass jeder konsistente Zielvorstellungen hat und die Entscheidungen logisch aus diesen Zielen ableitet. Für die Spieler ergibt sich daraus eine Nutzenmaximierung. Dies ist von Bedeutung, da dies die Voraussetzung für die Vorhersagbarkeit gegnerischer Züge bildet, um diese in der eigenen Entscheidungsfindung einzubeziehen. Es ist davon auszugehen, dass ein Othello-Spieler keinen Zug durchführen würde, der zu einem Verlust vieler eigener Steine führt, wenn ein alternativer Zug existiert \autocite[Kap.~2.6]{osborneIntroductionGameTheory2004}.

Aufbauend auf der antagonistischen Zielstruktur von Nullsummenspielen und der Rationalität der Spieler ergibt sich das Worst-Case-Prinzip. Da jeder vorteilhafte Zug eines Spielers einen Nachteil des anderen darstellt, ist bei der Bewertung des gegnerischen Zuges von der ungünstigsten Entscheidung auszugehen. Das daraus entstehende Minimax-Prinzip ist in Kapitel~\ref{sec:geeignete-algorithmen} genauer erläutert.

Ein weiteres zentrales Konzept ist die Strategie. Eine Strategie beschreibt dabei keinen einzelnen Zug, sondern eine vollständige Handlungsanweisung, die für jeden möglichen Spielzustand festlegt, welche Aktion ein Spieler ausführt. Die Spielermenge sei $N=\{1,2\}$ und die Strategiemenge $S_i$ von Spieler $i\in N$. Ein Strategienprofil $s=(s_1,s_2)\in S_1\times S_2$ bestimmt aufgrund der perfekten Information von Othello eindeutig den Spielverlauf und damit das Endergebnis. Dieses lässt sich durch eine Auszahlungsfunktion $u:S_1\times S_2\rightarrow\mathbb{R}^2$ beschreiben, wobei $u(s)=(u_1(s),u_2(s))$ den Auszahlungsvektor angibt und im Nullsummenfall $u_1(s)=-u_2(s)$ gilt. Theoretisch könnte das Ergebnis einer Othello Partie somit exakt bestimmt werden, sofern die vollständigen Strategien beider Spieler bekannt wären. In der Praxis ist dieser Ansatz jedoch nicht umsetzbar, da die Strategiemengen aufgrund der enormen Anzahl möglicher Spielzustände nicht explizit darstellbar sind. Dennoch besitzt der Strategiebegriff hohe Relevanz, da er die theoretische Grundlage für Entscheidungsprinzipien bildet. Suchverfahren versuchen daher nicht vollständige Strategien zu berechnen, sondern optimale Strategien mit begrenzten Ressourcen anzunähern \autocite[Kap.~2.6]{osborneIntroductionGameTheory2004}.

Des weiteren ist der Verzweigungsfaktor von Othello für die Analyse des Spielbaums relevant. Dieser bezeichnet die durchschnittliche Anzahl zulässiger Züge die einem Spieler im Laufe einer Partie zur Verfügung stehen. Er steht für die durchschnittliche Zahl an Nachfolgerknoten pro Knoten im Spielbaum. Eine Untersuchung der WTHOR Datenbank \autocite{BaseWTHORFederation} ergab, dass Othello einen durchschnittlichen Verzweigungsfaktor von $8.25$ und eine Spiellänge von $59.85$ Zügen besitzt. Ein durchschnittliches Spiel von Othello dauert somit fast so lang wie ein maximal langes mit $64~Felder-4~Anfangssteine=60~Zuege$. Es ergibt sich eine Spielbaumkomplexität von $8.25^{59.85}\approx 7.08\cdot10^{54}$ \autocite[Kap.~3.3.2]{nijssenMonteCarloTreeSearch2013}. Dies bedeutet, dass das der Spielbaum aufgrund des hohen Verzweigungsfaktors sehr breit wächst. Daraus folgend können nicht realistisch alle möglichen Zustände überprüft werden. Abgeleitet aus dem Verzweigungsfaktor wird zudem die Suchtiefe in der adversarialen Suche interessant. Obwohl das Spiel deterministisch ist, kann sich der exakte Spielverlauf nicht komplett vorhersagen, da man die gegnerischen Spielzüge nur bedingt durch die eigenen beeinflussen kann. Es wäre, selbst wenn es möglich wäre, nicht sinnvoll im Vorhinein alle möglichen Lösungswege zu bestimmen und nach jedem Zug abzuwägen, ob er weiter oder ein anderer verfolgt werden sollte. Durch die Bestimmung einer Suchtiefe kann der Suchaufwand erheblich reduziert werden, indem von jedem Zustand aus die $d$ darauffolgenden Zustände überprüft werden und unter diesen abgewogen wird. Bei 30 eigenen Zügen mit einer Suchtiefe von 3 und dem Verzweigungsfaktor müssten somit nur $30\cdot8.25^3\approx 16.800$ Zustände überprüft werden.
...
%Othello ist ein deterministisches Zwei-Personen-Nullsummenspiel mit perfekter Information. Diese Eigenschaften – kein Zufall, keine verdeckten Informationen – erlauben theoretisch eine vollständige Modellierung des Spiels durch Minimax-ähnliche adversariale Suchverfahren \autocite{russellArtificialIntelligenceModern2016}. In der Praxis ist der Spielbaum jedoch so groß, dass eine vollständige Exploration unmöglich ist: Die Anzahl möglicher Brettstellungen wird auf etwa $10^{28}$ geschätzt, und pro Stellung bestehen im Mittel rund 10 legale Züge. Damit ergeben sich schätzungsweise $10^{58}$ möglicher Partieverläufe, was jeden naiven Suchansatz sprengt \autocite{nijssenMonteCarloTreeSearch2013}.
%
%Klassische Minimax-Suchen sind formal korrekt, stoßen jedoch aufgrund dieser Komplexität schnell an ihre Grenzen. Alpha-Beta-Pruning kann den Suchaufwand drastisch reduzieren, da ungünstige Teilbäume frühzeitig abgeschnitten werden, wenn ihre Werte den aktuellen besten Pfad nicht beeinflussen können. Im Idealfall (bei perfekter Zugreihenfolge) sinkt der effektive Verzweigungsfaktor auf etwa die Quadratwurzel ab, doch auch ohne optimale Sortierung werden viele Knoten eingespart. Zur weiteren Beschleunigung verwenden Othello-Programme iterative Tiefensuchen (iterative deepening) und ausgefeilte Zuganordnungen (Move Ordering). Dabei werden Teilsuchergebnisse, Killer-Moves oder Historien-Heuristiken genutzt, um erfolgversprechende Züge zuerst zu prüfen. Transpositionstabellen verhindern doppelte Rechnungen, indem einmal untersuchte Stellungen zwischengespeichert werden.
%
%Da eine exakte Bewertung nur in Endstellungen möglich ist sind heuristische Bewertungsfunktionen zentral. Üblicherweise werden dafür die Steindifferenz der Spieler, Eckenkontrolle oder die Mobilität, im Sinne von Othello die Anzahl der zulässigen Züge im aktuellen Zustand, verwendet. Diese Einflussgrößen werden in gewichteten Summen kombiniert, sodass Ecken beispielsweise stark positiv bewertet werden. 
%
%Obwohl Othello deterministisch ist und perfekte Information besitzt, war das Spiel historisch lange ungelöst. Erst 2023 konnte der Spielwert formell bestimmt werden: Perfektes Spiel führt zu einem Remis \autocite{nijssenMonteCarloTreeSearch2013}. Tiefe lernbasierte Verfahren wie Reinforcement Learning mit neuronalen Netzen stoßen bei Othello auf praktische Grenzen, da die Größe des Zustandsraums und die Länge der Partien extremen Speicher- und Rechenaufwand erfordern. Die deterministische und symmetrische Struktur von Othello begünstigt daher klassische adversariale Suchverfahren und Monte-Carlo-Techniken, während rein daten- oder lernbasierte Ansätze nur unter hohem Ressourceneinsatz praktikabel sind. Bewertungsfunktionen und Suchheuristiken bleiben somit Kernbestandteile moderner Othello-KI-Systeme.
...
\section{Aufbau des LEGO\textsuperscript{\textregistered} Education SPIKE\texttrademark{} Prime Baukastensystems}
\label{sec:lego-spike}
Das LEGO\textsuperscript{\textregistered} Education SPIKE\texttrademark{} Prime Baukastensystem ist ein Lernsystem auf Basis von Klemmbausteinen des Herstellers LEGO\textsuperscript{\textregistered}. Es verfolgt ein praxisorientiertes didaktisches Konzept für den Schulunterricht in den Bereichen Mathematik, Informatik, Naturwissenschaften und Technik \autocite{legogroupLEGOEducationSPIKE}.
Das System umfasst Sensoren, Aktoren, einen Mikrocontroller sowie eine Entwicklungsumgebung. Es ist mit Komponenten des LEGO\textsuperscript{\textregistered} Technic\texttrademark{} Baukastensystems kompatibel und ermöglicht dadurch den Aufbau komplexerer mechatronischer Systeme.
Das SPIKE-System ist in zwei Ausführungen erhältlich, Essential und Prime. In dieser Arbeit wird die Prime-Variante betrachtet, auf die sich die Beschreibung beschränkt.

Die Grundlage des Systems ist die Möglichkeit, mehrere Bausteine und Komponenten über standardisierte Steckverbindungen miteinander zu verbinden. Hierfür steht eine Vielzahl von Kunststoffelementen in unterschiedlichen Geometrien zur Verfügung, darunter verschiedene Verbindungselemente, Zahnräder, Getriebekomponenten, Achsen, Räder, Ketten sowie weitere mechanische Bauteile. \autocite{conradelectronicseLEGOEducation45678}
Von besonderer Relevanz sind der programmierbare Hub als Mikrocontroller, die Entwicklungsumgebung sowie die verfügbaren Aktoren und Sensoren. Diese Komponenten bestimmen den Aufbau und die funktionalen Möglichkeiten des Roboters maßgeblich und stellen zugleich wesentliche technische Randbedingungen dar. Aus diesem Grund werden sie im Folgenden näher betrachtet. Dabei geht der Umfang der verfügbaren Bauteile über den des Basis-Sets hinaus.

\paragraph{Programmierbarer Hub}
Der programmierbare Hub (offizielle Bezeichnung: Technic\texttrademark{} Large Hub) bildet die zentrale Steuereinheit des Systems. Er stellt insgesamt sechs Ein- und Ausgänge zur Verfügung, an die Motoren und Sensoren angeschlossen werden können.
Zur integrierten Hardware gehört eine 5×5-LED-Matrix, die zur visuellen Anzeige von Statusinformationen dient. Weiterhin ist ein sechssachsiger Inertialsensor verbaut, der aus drei Achsen zur Erfassung linearer Beschleunigungen und drei Achsen zur Messung von Drehraten besteht.
Die Bedienung erfolgt über ein integriertes Taster-Navigationssystem, während akustische Rückmeldungen über einen eingebauten Lautsprecher ausgegeben werden können. Für die Kommunikation mit der Entwicklungsumgebung stehen eine USB-Schnittstelle sowie eine Bluetooth-Verbindung zur Verfügung. Die Energieversorgung erfolgt über einen herausnehmbaren, wiederaufladbaren Akku.
Die detaillierten technischen Spezifikationen sind dem Datenblatt in \autocite{legoreducationLEGOEducationSPIKE} zu entnehmen. Hervorzuheben ist der integrierte Prozessor mit einer Taktfrequenz von \SI{100}{\mega\hertz}. Zusätzlich stehen \SI{320}{\kilo\byte} Arbeitsspeicher sowie \SI{32}{\mega\byte} nichtflüchtiger Speicher für Programmcode und Daten zur Verfügung.

\paragraph{Entwicklungsumgebung}
Die Programmierung des Hubs erfolgt über die mitgelieferte Entwicklungsumgebung in Form einer dedizierten Programmier-App, die sowohl für mobile Endgeräte, insbesondere Tablets des Herstellers Apple, als auch für Desktop-Computer verfügbar ist. Die Entwicklungsumgebung unterstützt zwei Programmieransätze: eine blockbasierte visuelle Programmierung sowie eine textbasierte Programmierung auf Basis von MicroPython.
Für beide Varianten stehen zahlreiche vorgefertigte Module und Funktionsbausteine zur Verfügung, die den Zugriff auf Sensoren, Aktoren und weitere Systemkomponenten ermöglichen. Die zugehörigen Programmierschnittstellen sind umfangreich dokumentiert und werden direkt innerhalb der Entwicklungsumgebung bereitgestellt \autocite{legoreducationLEGOEducationSPIKEa}.

\paragraph{Großer und mittelgroßer Winkelmotor}
Es stehen insgesamt drei große und drei mittelgroße Winkelmotoren (offizielle Bezeichnungen: Technic\texttrademark{} Medium Angular Motor und Technic Large Angular Motor) mit integriertem Rotationssensor zur Verfügung. Die beiden Motortypen unterscheiden sich insbesondere in ihrer Geometrie, ihrem Eigengewicht sowie dem verfügbaren Drehmoment.  
Beide Varianten ermöglichen die Erfassung sowohl relativer als auch absoluter Positionen im Bereich von $\pm \SI{180}{\degree}$. Die Auflösung beträgt $360$ Zählwerte pro Umdrehung. Die Positionsgenauigkeit liegt bei höchstens $\pm \SI{3}{\degree}$ und ergibt sich im Wesentlichen aus Sensortoleranzen sowie mechanischem Spiel im Getriebe. Die Aktualisierungsrate der Positionsmessung beträgt $\SI{100}{\hertz}$.  
Weiterführende und detaillierte Angaben, insbesondere zur Motorleistung, sind den jeweiligen Datenblättern~\autocite{legoreducationLEGOEducationSPIKEb} und~\autocite{legoreducationLEGOEducationSPIKEc} zu entnehmen.

\paragraph{Abstandssensor}
Es steht ein Abstandssensor (offizielle Bezeichnung: Technic\texttrademark{} Distance Sensor) zur Verfügung, der zur berührungslosen Messung von Distanzen zu Objekten mittels Ultraschall dient. Der Sensor besitzt einen Öffnungswinkel von $\pm \SI{35}{\degree}$ und eine Auflösung von $\SI{1}{\milli\metre}$.  
Es stehen zwei Messmodi zur Verfügung: ein Normalmodus sowie ein Schnellmodus. Im Normalmodus können Entfernungen im Bereich von $\SI{50}{\milli\metre}$ bis $\SI{2000}{\milli\metre}$ mit einer Messgenauigkeit von $\pm \SI{20}{\milli\metre}$ erfasst werden. Der Schnellmodus ist auf einen Messbereich von $\SI{50}{\milli\metre}$ bis $\SI{300}{\milli\metre}$ begrenzt und weist eine Genauigkeit von $\pm \SI{15}{\milli\metre}$ auf.  
Weiterführende technische Angaben sind dem zugehörigen Datenblatt~\autocite{legoreducationLEGOEducationSPIKEd} zu entnehmen.

\paragraph{Kraftsensor}
Es steht ein Kraftsensor (offizielle Bezeichnung: Technic\texttrademark{} Force Sensor) zur Verfügung, mit dem Berührungs-, Tipp- und Kraftzustände erfasst werden können. Der Sensor unterstützt drei Betriebsarten: Berührungs-Modus, Tipp-Modus und Kraft-Modus.  
Im Berührungs-Modus beziehungsweise Tipp-Modus liegt die Aktivierungszone im Bereich von $\SI{0}{\milli\metre}$ bis $\SI{2}{\milli\metre}$. Der Schwellwert ist firmwareseitig auf $\SI{1}{\milli\metre}$ mit einer Toleranz von $\pm \SI{0.5}{\milli\metre}$ festgelegt. Die erforderliche Aktivierungskraft beträgt $\SI{0.5}{\newton}$ bis $\SI{1.0}{\newton}$ bei einer Toleranz von $\pm 10\,\%$. Im Berührungs-Modus erfolgt die Sensorausgabe binär, wobei der Zustand entweder aktiviert oder nicht aktiviert ist. Im Tipp-Modus wird ein diskreter Ausgabewert im Bereich von $0$ bis $3$ bereitgestellt.  
Im Kraft-Modus ermöglicht der Sensor die Messung aufgebrachter Kräfte. Die Aktivierungszone liegt hierbei zwischen $\SI{2}{\milli\metre}$ und $\SI{8}{\milli\metre}$, während der messbare Kraftbereich von $\SI{2.5}{\newton}$ bis $\SI{10}{\newton}$ reicht. Die Auflösung der Sensorausgabe beträgt $\SI{0.1}{\newton}$, wobei der maximale Ausgabewert auf $\SI{10}{\newton}$ begrenzt ist. Die Messgenauigkeit liegt bei $\pm \SI{0.65}{\newton}$.  
Detaillierte Angaben sind dem zugehörigen Datenblatt~\autocite{legoreducationLEGOEducationSPIKEe} zu entnehmen.

\paragraph{Farbsensor}
Es stehen zwei Farbsensoren (offizielle Bezeichnung: Technic\texttrademark{}  Color Sensor) zur Verfügung. Diese ermöglichen die Erfassung von Farben, der Oberflächenreflektivität sowie der Umgebungshelligkeit.  
Die optimale Messdistanz beträgt etwa $\SI{16}{\milli\metre}$ und ist abhängig von der Objektgröße, der Farbe sowie der Oberflächenbeschaffenheit. Die Farberkennung erfolgt wahlweise über RGB- und HSV-Werte oder über vordefinierte Farbkategorien.  
Die Reflektivitätsmessung liefert Ausgabewerte im Bereich von $\SI{0}{\percent}$ für nicht reflektierende Oberflächen bis $\SI{100}{\percent}$ für stark reflektierende Oberflächen. Die Umgebungslichtmessung gibt Helligkeitswerte zwischen $\SI{0}{\percent}$ für dunkle und $\SI{100}{\percent}$ für helle Umgebungen aus.  
Weitere detaillierte Angaben sind dem zugehörigen Datenblatt~\autocite{legoreducationLEGOEducationSPIKEf} zu entnehmen.

\section{Grundlagen robotischer Systeme}
\label{sec:roboter}
Die Definition eines Roboters oder eines robotischen Systems variiert stark. Eine weit verbreitete und für diese Arbeit geeignete Definition liefert die VDI-Richtlinie 2860. Diese definiert Industrieroboter wie folgt:
\glqq Industrieroboter sind universell einsetzbare Bewegungsautomaten mit mehreren Achsen, deren Bewegungen hinsichtlich Bewegungsfolge und Wegen bzw. Winkeln frei (d.h. ohne mechanischen Eingriff) programmierbar und ggf. sensorgeführt sind. Sie sind mit Greifern, Werkzeugen oder anderen Fertigungsmitteln ausrüstbar und können Handhabungs- und/oder Fertigungsaufgaben ausführen.\grqq{} \autocite[Kap.~2.1]{naberGrundlagenMobilerRoboter1991}.
Demnach bestehen robotische Systeme aus den folgenden drei Kernelementen:

\begin{itemize}
	\item \textbf{Mechanischer Aufbau:} Bestehend aus Struktur, Gelenken und Endeffektoren. Diese Komponenten ermöglichen die physische Interaktion des Roboters mit seiner Umgebung und die Ausführung spezifischer Aufgaben.
	\item \textbf{Sensorik:} Dient der Wahrnehmung der Umgebung und des eigenen Zustands. Beispielhafte Sensoren sind Kraft-/Drehmomentsensoren, Kameras oder Lidar.
	\item \textbf{Aktorik:} Ermöglicht die Bewegung und Manipulation der mechanischen Komponenten, z.~B. durch Motoren, Servoantriebe oder Hydrauliksysteme.
\end{itemize}

Darüber hinaus besitzen robotische Systeme in der Regel eine \textbf{Steuereinheit}, die die Verarbeitung von Sensordaten übernimmt, Bewegungen berechnet und die Aktoren ansteuert. 

Roboter lassen sich auf verschiedene Weise klassifizieren. \autocite{corkeRoboticsControlFundamental2022} unterscheidet robotische Systeme insbesondere nach mehreren Kriterien. Dazu gehören die Mobilität, also ob ein Roboter stationär oder mobil ist, sowie die Bewegungsfreiheit beziehungsweise die Anzahl der Freiheitsgrade. Auch der Einsatzbereich, etwa in der Industrie, Forschung, im Haushalt oder im medizinischen Umfeld, dient als Unterscheidungskriterium. Schließlich kann die Kinematik der Roboter berücksichtigt werden, wobei zwischen seriellen, parallelen oder hybriden Strukturen unterschieden wird.


Robotische Systeme lassen sich auf unterschiedlichste Weise aufbauen. In der Industrie haben sich jedoch bestimmte Standardaufbauten etabliert, da diese die algorithmische Berechnung der Rückwärtskinematik vereinfachen.
Im Folgenden werden drei für diese Arbeit relevante Aufbauarten von Robotern vorgestellt: SCARA, PUMA und Gantry.

\paragraph{SCARA-Roboter} 
SCARA steht für \emph{Selective Compliance Assembly Robot Arm}. Diese Roboter besitzen typischerweise vier Achsen, wobei die ersten beiden in der Horizontalebene rotieren und die dritte Achse die vertikale Bewegung ausführt. SCARA-Roboter zeichnen sich durch hohe Geschwindigkeit, Präzision und Wiederholgenauigkeit in der Horizontalbewegung aus, wodurch sie insbesondere für Montage- und Bestückungsaufgaben geeignet sind. Ihre Struktur erlaubt eine gewisse Flexibilität in der horizontalen Ebene, während die vertikale Achse starr geführt ist. \autocite[S.~189~f.]{corkeRoboticsControlFundamental2022}

\paragraph{PUMA-Roboter} 
Der PUMA-Roboter (\emph{Programmable Universal Machine for Assembly}) ist ein klassischer, serieller Roboterarm mit sechs Achsen. Durch die serielle Anordnung der Gelenke ist er äußerst flexibel und kann komplexe Bewegungen im dreidimensionalen Raum ausführen. PUMA-Roboter finden vor allem in der Montage, Handhabung und Fertigung Anwendung, wo eine hohe Bewegungsfreiheit und präzise Positionierung erforderlich sind. Die Rückwärtskinematik ist bei standardisierten PUMA-Strukturen gut lösbar, was die Programmierung vereinfacht. \autocite[S.~194~f.]{corkeRoboticsControlFundamental2022}

\paragraph{Gantry-Roboter} 
Gantry-Roboter, auch Portalroboter genannt, bestehen aus linearen Achsen, die eine Arbeitsplattform in kartesischen Koordinaten bewegen. Typischerweise besitzen sie drei orthogonale Achsen (X, Y, Z), können aber um zusätzliche Achsen für Rotationen erweitert werden. Gantry-Roboter bieten eine hohe Tragfähigkeit und erreichen große Arbeitsbereiche bei vergleichsweise einfacher Mechanik. Sie werden häufig für Pick-and-Place-Aufgaben, Lagerautomation oder das Handling schwerer Werkstücke \autocite[S.~189~f.]{corkeRoboticsControlFundamental2022}.
